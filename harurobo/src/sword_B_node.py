...
def controller_callback(data):
    global ctrl_data
    ctrl_data = data
    if ctrl_data.Start and not(ctrl_data.Y or ctrl_data.A or ctrl_data.X):
        init_sword()
    elif ctrl_data.B and (ctrl_data.RT or ctrl_data.LT):
...
def init_sword():
    ...
    if result.success:
        rospy.loginfo("success : sword_B intialiation")
    else:
        rospy.loginfo("fail : sword_B intialiation")

class Auto_Sword():
    """ペットボトルハンドを自動で動かす"""
    global ctrl_data
    def __init__(self):
        self.action_client = actionlib.SimpleActionClient('auto_sword_B', HandDataAction)
        self.goal = HandDataGoal()
        self.rate = rospy.Rate(10)
    def sword_vertical(self):
        """ハンドを縦向きにして腕を下ろす"""
        if ctrl_data.Back:
            rospy.loginfo("interrupt : auto sword B")
            return
    ...
    def release_sword1(self):
        ...
        if ctrl_data.Back:
            rospy.loginfo("interrupt : release sword B")
            return
...
class ManualSword():
    ...
    global ctrl_data
    def __init__(self):
        self.manual_pub = rospy.Publisher('sword_B_manual_data', ManualHand, queue_size=10)
